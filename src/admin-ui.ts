...
(() => {
  ...
  function renderGrants() {
    ...
    const grants = state.grants.filter((grant) => JSON.stringify(grant).toLowerCase().includes(query) || grant.owners.some((id) => userName(id).toLowerCase().includes(query)));
    list.replaceChildren();
    if (!grants.length) {
    ...
    grants.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).forEach((grant) => {
      ...
      const title = node('div', { class: 'grant-title' }, [node('h3', { text: grant.itemId, title: grant.itemId }), node('span', { class: 'type', text: grant.mediaType || 'media' })]);
      ...
      const main = node('div', { class: 'grant-main' }, [title, node('div', { class: 'meta', text: sourceText + ' · updated ' + new Date(grant.updatedAt).toLocaleString() })]);
      const ownerSection = node('div', { class: 'grant-section' }, [node('strong', { text: 'Access owners' }), node('div', { class: 'pill-row' }, ownerPills.length ? ownerPills : [node('span', { class: 'meta', text: 'No direct owners' })])]);
      const accessSection = node('div', { class: 'grant-section' }, [node('strong', { text: groupNames.length ? groupNames.join(', ') : 'Direct requests only' }), node('div', { class: 'pill-row' }, [node('span', { class: 'pill sync-' + grant.sync.state, text: grant.sync.state }), node('span', { class: 'meta', text: groupNames.length + ' shared group' + (groupNames.length === 1 ? '' : 's') })])]);
      ...
      list.append(node('article', { class: 'grant-card' }, [main, ownerSection, accessSection, actions]));
    });
  }
  ...
  byId('library-previous').addEventListener('click', () => { state.catalogPage -= 1; renderCatalog(); });
  byId('library-next').addEventListener('click', () => { state.catalogPage += 1; renderCatalog(); });
